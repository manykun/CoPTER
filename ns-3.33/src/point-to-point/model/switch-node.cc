...
void SwitchNode::SwitchNotifyDequeue(uint32_t ifIndex, uint32_t qIndex, Ptr<Packet> p){
	...
	// update rate stats
	uint64_t nowTs = Simulator::Now().GetNanoSeconds();
	...
	UpdatePortRate(ifIndex, pktSize, nowTs);
	if (ecnMarked){
		UpdatePortEcnRate(ifIndex, pktSize, nowTs);
	}
...
void SwitchNode::UpdatePortRate(uint32_t ifIndex, uint32_t size, uint64_t ts){
	RateStats &rs = m_rateStats[ifIndex];
	if (rs.pktCount == 0){
		rs.startTs = ts;
	}
	rs.pktCount++;
...
void SwitchNode::UpdatePortEcnRate(uint32_t ifIndex, uint32_t size, uint64_t ts){
	RateStats &rs = m_rateStats[ifIndex];
	rs.totalEcnBytes += size;
	rs.endTs = ts; // no use
}

double SwitchNode::GetPortEcnRate(uint32_t ifIndex){
	RateStats &rs = m_rateStats[ifIndex];
	if (rs.pktCount == 0 || rs.endTs <= rs.startTs)
		return 0.0;
	double timediff = rs.endTs - rs.startTs;
	double rate = rs.totalEcnBytes * 8.0 / timediff; // bps
	rate = rate * 1e9;
	return rate;
}

double SwitchNode::GetPortRate(uint32_t ifIndex){
	RateStats &rs = m_rateStats[ifIndex];
	if (rs.pktCount == 0 || rs.endTs <= rs.startTs)
		return 0.0;
	double timediff = (rs.endTs - rs.startTs);
	double rate = rs.totalBytes * 8.0 / timediff; // bps
	rate = rate * 1e9;
	return rate;
}

void SwitchNode::ResetRateStats(uint32_t ifIndex, uint64_t ts) {
	RateStats &rs = m_rateStats[ifIndex];
	rs.pktCount = 0;
	rs.totalBytes = 0;
	rs.totalEcnBytes = 0;
...
void SwitchNode::ResetAllRateStats(uint64_t ts) {
	for (uint32_t i = 0; i < pCnt; i++) {
		ResetRateStats(i, ts);
	}
}
