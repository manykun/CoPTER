...
/************************************************
 * RDMA configuration varibles
 ***********************************************/
uint32_t cc_mode = 1;
bool enable_copter = false, enable_monitor = false, enable_qcn = true, use_dynamic_pfc_threshold = true;
 ...
 * OpenGym Varibles
 ***********************************************/
uint32_t opengym_socket_port = 5555;
double opengym_step_time = 0.0005; // 500 us
double opengym_monitor_interval = 0.0001; // 100 us
double opengym_end_time = 0xdeafbeef;
NodeContainer opengym_switches_pool; // all switches in the topology
uint32_t opengym_n_switches = 0; // total number of switches in the topology
...
Ptr<OpenGymSpace> MyGetObservationSpace(void) {
    ...
    float low = 0.0;
    float high = 1.0;
    std::vector<uint32_t> shape = {opengym_n_ports * opengym_n_state_per_port};
...
Ptr<OpenGymSpace> MyGetActionSpace(void) {
    NS_LOG_UNCOND("OpenGym Action Space = " << opengym_n_ports << " * " << opengym_n_action_per_port << " = " << opengym_n_ports * opengym_n_action_per_port << " values");
    float low = 0.0;
    float high = 1.0;
    std::vector<uint32_t> shape = {opengym_n_ports * opengym_n_action_per_port};
    std::string dtype = TypeNameGet<double> ();
    Ptr<OpenGymBoxSpace> space = CreateObject<OpenGymBoxSpace> (low, high, shape, dtype);
    return space;
}
...
Ptr<OpenGymDataContainer> MyGetObservation(void) {
    ...
    for (uint32_t i = 0; i < opengym_n_switches; i++ ) {
        ...
        for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
            ...
            double min_kmin = opengym_min_kmin * (max_port_rate / 25e9);
            double max_kmin = opengym_max_kmin * (max_port_rate / 25e9);
            double min_kmax = opengym_min_kmax * (max_port_rate / 25e9);
            double max_kmax = opengym_max_kmax * (max_port_rate / 25e9);

            double norm_kmin = (sw->m_mmu->kmin[j] - min_kmin) / (max_kmin - min_kmin);
            double norm_kmax = (sw->m_mmu->kmax[j] - min_kmax) / (max_kmax - min_kmax);

            box->AddValue(norm_kmin);
            box->AddValue(norm_kmax);
...
float MyGetReward(void) {
    ...
    // A problem may be how to have the queue length-D(L) mapping from python side - we need to explicitly set the buffer size in the python side.
    return 0;
}
...
bool MyExecuteActions(Ptr<OpenGymDataContainer> action) {
    ...
    for (uint32_t i = 0; i < opengym_n_switches; i++ ) {
        ...
        for (uint32_t j = 1; j < sw->GetNDevices(); j++) {
            ...
            // Convert normalized ECN threshold to byte value according to port rate
            uint64_t max_port_rate =  DynamicCast<QbbNetDevice>(sw->GetDevice(j))->GetDataRate().GetBitRate();
            double min_kmin = opengym_min_kmin * (max_port_rate / 25e9);
            double max_kmin = opengym_max_kmin * (max_port_rate / 25e9);
            double min_kmax = opengym_min_kmax * (max_port_rate / 25e9);
            ...
            // Note `ConfigEcn` requires KB input.
...
void ReadFlowInput(){
    if (flow_input.idx < flow_num){
        ...
        // set the end time to the last flow start time 
        // I think this will reduce the effect of "reduced" traffic
    }
}
...
int main(int argc, char *argv[])
{
...
#endif
    {
        ...
        while (!conf.eof())
        {
            ...
            //std::cout << conf.cur << "\n";
            if (key.compare("ENABLE_COPTER") == 0) {
                uint32_t v;
                conf >> v;
                enable_copter = v;
                if (enable_copter)
                    std::cout << "ENABLE_COPTER\t\t\t" << "Yes" << "\n";
                else
                    std::cout << "ENABLE_COPTER\t\t\t" << "No" << "\n";
            }
                ...
                enable_monitor = v;
                if (enable_monitor)
                    std::cout << "ENABLE_MONITOR\t\t\t" << "Yes" << "\n";
                else
                    std::cout << "ENABLE_MONITOR\t\t\t" << "No" << "\n";
            }
            ...
            }else if (key.compare("QLEN_MONITOR_FILE") == 0) {
                conf >> qlen_monitor_file;
                std::cout << "QLEN_MONITOR_FILE\t\t\t" << qlen_monitor_file << '\n';
            }else if (key.compare("RATE_MONITOR_FILE") == 0) {
                conf >> txrate_monitor_file;
                std::cout << "RATE_MONITOR_FILE\t\t\t" << txrate_monitor_file << '\n';
    ...
    for (uint32_t i = 0; i < node_num; i++){
        if (n.Get(i)->GetNodeType() == 1){ // is switch
            ...
            sw->m_mmu->ConfigBufferSize(buffer_size *1024); // according to m3 implementation
            sw->m_mmu->node_id = sw->GetId();

            sw->ResetAllRateStats(2000000000);
        }
    }
    ...
    opengym_n_switches = opengym_switches_pool.GetN();
    
    if (enable_copter) {
        for (uint32_t i = 0; i < opengym_n_switches; i++) {
            if (opengym_switches_pool.Get(i)->GetNodeType() == 1) { // is switch
                Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(opengym_switches_pool.Get(i));
        ...
        openGym->SetGetExtraInfoCb(MakeCallback (&MyGetExtraInfo));
        openGym->SetExecuteActionsCb(MakeCallback (&MyExecuteActions));
        Simulator::Schedule(Seconds(2.000 + 2 * opengym_step_time), &ScheduleNextStateRead, opengym_step_time, openGym);
    }
